...
import {userDataContext} from './UserContext'
 ...
 function ShopContext({children}) {
  ...
  const addToCart = async (itemId, size) => {
    ...
    if(userData){
      try {
        let result = await axios.post(serverUrl + '/api/cart/add',{itemId,
          size}, {withCredentials:true})
          console.log(result.data)
      } catch (error) {
        console.log(error)
  ...
  const updateQuantity = async (itemId, size, quantity) => {
    ...
    if(userData){
    try {
      await axios.post(serverUrl+ "/api/cart/update",{itemId,
        size,quantity}, {withCredentials:true})
    } catch (error) {
      console.log(error)
