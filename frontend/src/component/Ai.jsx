...
function Ai() {
  ...
  recognition.onresult = (e)=>{
    ...
    if(transcript.toLowerCase().includes("search") && transcript.toLowerCase().includes("open") && !showSearch){
      ...
      navigate('/')
      setShowSearch(false)
    }else if(transcript.toLowerCase().includes("cart") || transcript.toLowerCase().includes("caat") || transcript.toLowerCase().includes("kaat")){
  ...
  return (
    <div className='fixed lg:bottom-[20px] md:bottom-[40px] bottom-[90px] left-[2%]' onClick={()=>{recognition.start();
    setActiveAi(true)}}>
      <img src={ai} alt='' className={`w-[70px] cursor-pointer rounded-4xl ${activeAi ? 'translate-x-[10%] translate-y-[10%] scale-125' : 'translate-x-[0] translate-y-[0] scale-100'} transition-transform` } style={{filter: `${activeAi ? 'drop-shadow(0px 0px 30px #00d2fc)' :'drop-shadow(0px 0px 20px black)'}`}}/>
    </div>
  )
}
