...
function Collections() {
  ...
  const sortProducts = (e)=>{
    ...
    switch(sortType){
      case 'low-high':
        setFilterProduct(fbCopy.sort((a,b)=>
          (a.price - b.price)))
        break;
    ...
    md:flex-row justify-start pt-[70px] overflow-x-hidden
     z-[2] pb-[110px]'>
      ...
      lg:fixed ${showFilter ? "h-[45vh]" : "h-[8vh]"}  `}>
        ...
        md:block`}>
          ...
          justify-center gap-[10px] flex-col'>
            ...
            text-[16px] font-light' > <input 
            type='checkbox' value={'Men'} className='w-3' 
            onChange={toggleCategory}/>Men</p>

            <p className='flex items-center justify-center gap-[10px]
            text-[16px] font-light' > <input 
            type='checkbox' value={'Women'} className='w-3' 
            onChange={toggleCategory}/>Women</p>

            <p className='flex items-center justify-center gap-[10px]
            text-[16px] font-light' > <input 
            type='checkbox' value={'kids'} className='w-3' 
            onChange={toggleCategory}/>Kids</p>
          </div>
        </div>
        ...
        md:block`}>
          ...
          justify-center gap-[10px] flex-col'>
            <p className='flex items-center justify-center gap-[10px]
            text-[16px] font-light' > <input 
            type='checkbox' value={'TopWear'} className='w-3' 
            onChange={toggleSubCategory}/>TopWear</p>

            <p className='flex items-center justify-center gap-[10px]
            text-[16px] font-light' > <input 
            type='checkbox' value={'BottomWear'} className='w-3'
            onChange={toggleSubCategory}/>BottomWear</p>

            <p className='flex items-center justify-center gap-[10px]
            text-[16px] font-light' > <input 
            type='checkbox' value={'WinterWear'} className='w-3' 
      ...
      <div className='lg:pl-[20%] md:py-[10px]'>
        ...
        p-[20px] flex justify-between flex-col lg:flex-row
         lg:px-[50px]'>
          <Title text1={"ALL"} text2={"COLLECTIONS"} />

          <select name='' id='' className='bg-slate-600 w-[60%]
          md:w-[200px] h-[50px] px-[10px] text-[white] rounded-lg
          hover:border-[#461f7] border-[2px]' onChange={(e)=>setSortType(e.target.value)}>
            <option value="relavent" className='w-[100%] h-[100%]'>
              Sort By: Relavent</option>
